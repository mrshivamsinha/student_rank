...
import { motion } from "framer-motion";
import topperList from "../data/topperList.json";
...
const RankDetails = () => {
  ...
  useEffect(() => {
    const fetchStudentAndTopRankers = async () => {
      try {
        // Simulate loading for demo purposes
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Fetch the searched student
        const docRef = doc(db, "students", regNo);
        const docSnap = await getDoc(docRef);
        ...
        // Find top 3 students from same branch and batch
        const filteredTopStudents = topperList.filter(
          s => s.Branch === studentData.Branch && 
               s.Batch === studentData.Batch && 
               s["Registration Number"] !== regNo
        ).slice(0, 3);
        
        setTopStudents(filteredTopStudents);
      } catch (err) {
  ...
  <span className="ml-2 text-sm">CGPA: {student["Cur. CGPA"]}</span>
          ...
          <div className="p-6 md:p-8">
            ...
            {topStudents.length > 0 && (
              <div className="mt-8">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Top Rankers</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {topStudents.map((topStudent, index) => (
                    <motion.div
                      key={topStudent["Registration Number"]}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600">
                        <p>Reg No: {topStudent["Registration Number"]}</p>
                        <p className="mt-1">
                          CGPA: <span className="font-semibold">{topStudent["Cur. CGPA"]}</span>
                        </p>
                      </div>
                    </motion.div>
                  ))}
